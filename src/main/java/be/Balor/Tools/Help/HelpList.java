 ...
 */
class HelpList {
	private TreeSet<HelpEntry> pluginHelp = new TreeSet<HelpEntry>(
	...
	public boolean removeEntry(final String commandName) {
		DebugLog.INSTANCE.info("Remove " + commandName + " help from plugin : "
				+ pluginName);
	...
	public HelpList(final Plugin plugin) throws IllegalArgumentException {
		final TreeSet<HelpEntry> list = new TreeSet<HelpEntry>(
				new EntryNameComparator());
		final Map<String, Map<String, Object>> cmds = plugin.getDescription()
				.getCommands();
		this.pluginName = plugin.getDescription().getName();
		if (cmds == null) {
			throw new IllegalArgumentException(pluginName
					+ " don't have any commands to list");
		}
		...
		try {
			...
			if (ConfigEnum.VERBOSE.getBoolean()) {
				ACLogger.warning("[HELP] Problem with commands of "
						+ pluginName);
			}
	...
	public List<String> getPage(int page, final CommandSender sender,
		...
		final int maxPages = (int) Math.ceil(lastHelpEntries.size()
				/ (double) entryPerPage);
		page = page > maxPages ? maxPages : page;
		final int start = (page - 1) * entryPerPage;
		final int end = start + entryPerPage > lastHelpEntries.size()
				? lastHelpEntries.size()
				: start + entryPerPage;
	 ...
	 */
	public List<HelpEntry> getCommandMatch(final String cmd,
		...
		if (lastCommandSearched != null
				&& lastCommandSearched.getCmd().equals(cmd)) {
